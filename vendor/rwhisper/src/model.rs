            ...
            while let Some((index, token)) = remaining_tokens.pop() {
                ...
                if let Some(timestamps) = &token_timestamps {
                    if timestamp_start.is_none() {
                        // DTW can occasionally return fewer timestamps than
                        // decoded tokens (notably on long multilingual input).
                        // Clamp trailing tokens to the final known timestamp
                        // instead of panicking the transcription worker.
